...
public class AIDAcompare {
    ...
    public static void main(String[] args) throws Exception {
        ...
        for (Document document : corpus) {
            try {
                ...
                // if (document.getDocumentId() != 616
                // && document.getDocumentId() != 698
                // && document.getDocumentId() != 902
                // && document.getDocumentId() != 1058
                // && document.getDocumentId() != 1096
                // && document.getDocumentId() != 1111
                // && document.getDocumentId() > 1116)
                {
                    String text = markupText(document);
                    // Prepare the input for disambiguation. The Stanford NER
                    // will
                    // be run
                    // to identify names. Strings marked with [[ ]] will also be
                    // treated as names.
                    ...
                    NamedEntitiesInText namedEntities = document
                            .getProperty(NamedEntitiesInText.class);

                    for (NamedEntityInText namedEntity : namedEntities) {
                        if (namedEntity.getLength() > 2) {
                            String correctVotingURL = namedEntity
                                    .getNamedEntityUri();
                            ...
                            String disambiguatedURL = findResult(
                                    namedEntity.getStartPos(), results);
                            if (correctVotingURL != null) {
                                if (correctVotingURL.equals(disambiguatedURL)) {
                                    t++;
                                    log.info("\t Disambiguated: "
                                            + correctVotingURL + " -> "
                                            + disambiguatedURL);
                                } else if (correctVotingURL
                                        .equals("http://aksw.org/notInWiki")
                                        || correctVotingURL
                                                .startsWith("http://rdflivenews.aksw.org/resource/")
                                        || correctVotingURL
                                                .startsWith("http://de.dbpedia.org/")) {
                                    log.info("\t Closed World Assumption: "
                                            + correctVotingURL + " -> "
                                            + disambiguatedURL);
                                } else {
                                    n++;
                                    log.info("\t Not disambiguated: "
                                            + correctVotingURL + " -> "
                                            + disambiguatedURL);
                                }
                                ...
                                String label = document
                                        .getProperty(DocumentText.class)
                                        .getText().substring(start, end);
                                n++;
                                log.info("\t No candidates: " + label + " -> "
                                        + correctVotingURL);
                            } else {
                                log.info("\t Strange: " + correctVotingURL
                                        + " -> " + disambiguatedURL);
                            }
                        }
                    }
                }
                System.out.println("time:\t" + (System.currentTimeMillis() - starttime) + "\tentities:\t" + document
                        .getProperty(NamedEntitiesInText.class).getNamedEntities().size());
            } catch (Exception e) {
                log.error(document.getDocumentId() + " "
    ...
    private static String findResult(int startPos, DisambiguationResults results) {
        ...
        for (ResultMention rm : results.getResultMentions()) {
            ...
            if (rm.getCharacterOffset() == startPos) {
                ...
                return wikiURL.replace("http://en.wikipedia.org/wiki",
                        "http://dbpedia.org/resource");
            }
        }
        return null;
    ...
    private static String markupText(Document document) {
        ...
        List<NamedEntityInText> labels = document.getProperty(
                NamedEntitiesInText.class).getNamedEntities();
        Collections.sort(labels);
        String originalText = document.getProperty(DocumentText.class)
                .getText();
        ...
        for (NamedEntityInText entity : labels) {
            ...
            if (startFormerLabel >= entity.getEndPos()) {
                ...
                textParts.add(originalText.substring(entity.getEndPos(),
                        startFormerLabel));
                // append the markedup label
                String label = originalText.substring(entity.getStartPos(),
                        entity.getEndPos());
                textParts.add("[[" + label + "]]");
                // remember the start position of this label
                startFormerLabel = entity.getStartPos();
            } else {
                log.error("Label undercuts another label. TextId: "
