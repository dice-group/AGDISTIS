...
public class TripleIndexTest {
  ...
  public void testType() {
    ...
    final List<Triple> type =
        index.search(candidateURL, "http://www.w3.org/1999/02/22-rdf-syntax-ns#type", null);
  ...
  public void testRdfsLabel() {
    ...
    final String candidateURL = "http://dbpedia.org/resource/Barack_Obama";
    final List<Triple> type =
        index.search(candidateURL, "http://www.w3.org/2000/01/rdf-schema#label", null);
    assertTrue(type.size() > 0);
    for (final Triple t : type) {
      log.debug(t.toString());
    }
   ...
   * "http://dbpedia.org/property/acronym", null); assertTrue(type.size() > 0); for (Triple t :
   * type) { log.debug(t.toString()); } }
   */
  @Test
  ...
  public void testSurfaceFormsDistance() {
    final String candidateURL = "http://dbpedia.org/resource/Barack_Obama";
    final List<Triple> label =
        index.search(candidateURL, "http://www.w3.org/2000/01/rdf-schema#label", null);
    final List<Triple> surfaceForms =
        index.search(candidateURL, "http://www.w3.org/2004/02/skos/core#altLabel", null);
